...
> Версия с разбивкой на части для удобной работы в Cursor: `docs/documentation/README.md`
...
12. [Работа с мок-сервером и сценариями x-scenario](#12-работа-с-мок-сервером-и-сценариями-x-scenario)
13. [Тестирование и Storybook](#13-тестирование-и-storybook)
14. [Чек-лист для нового модуля](#14-чек-лист-для-нового-модуля)
15. [Типичный сценарий: экран «Каталог»](#15-типичный-сценарий-экран-каталог)
16. [Error Boundary (обработка ошибок рендера)](#16-error-boundary-обработка-ошибок-рендера)
17. [Планирование фичи в трекере (milestone и issues)](#17-планирование-фичи-в-трекере-milestone-и-issues) — опционально, по явному запросу в промпте
18. [Реализация фичи и поставка (ветка, PR, закрытие задач)](#18-реализация-фичи-и-поставка-ветка-pr-закрытие-задач) — опционально, по явному запросу в промпте
...
Контейнер соединяет данные из Redux (селекторы, `dispatch`) с UI. **В контейнере держите минимум разметки:** не размещайте в нём объёмную вёрстку, длинные цепочки JSX и styled-обёртки. Экраны, карточки, секции, кнопки и формы выносите в компоненты в `components/`; в контейнере оставляйте только композицию — несколько дочерних компонентов и передачу пропсов и обработчиков.
...
Для разработки и проверки разных ответов API (успех, ошибки 400/404/500, пустой ответ) используется [мок-сервер](https://github.com/AlexeyAbretov/ProjectTemplateFrontendSpaReact-mock-server). Он поддерживает сценарии через заголовок **`x-scenario`**: по значению заголовка обработчик на моке возвращает нужный статус и тело ответа.

### Как это устроено на фронтенде

В режиме разработки (`npm run start`) в качестве HTML-точки входа используется **`public/develop.html`** (в продакшене — `index.html`). В нём переопределён **`window.fetch`**:

1. При вызове `fetch(resource, config)` первый аргумент **`resource`** — это **pathname без протокола и домена** (то есть путь запроса: например, `api/v1/module1/list` или полный путь в зависимости от того, как вызывается `fetch` в коде).
2. Если `resource` содержит подставленный при сборке `API_PATH` (например, `api/v1/`), запрос считается обращением к API.
3. Для таких запросов строится ключ сценария в **localStorage**:
   - **Ключ:** `{METHOD}-{path}`, где путь берётся из `resource`: слэши заменяются на дефисы.  
     Пример: для `GET` и `resource = "api/v1/module1/list"` ключ: **`GET-api-v1-module1-list`**.
4. Значение по этому ключу читается из `localStorage` и подставляется в заголовок **`x-scenario`** каждого запроса к API. Мок-сервер по этому заголовку выбирает сценарий ответа (успех, `http400`, `http500`, `empty` и т.д.).

Таким образом, чтобы проверить, например, обработку 400 на эндпоинте `GET /api/v1/module1/list`, в консоли браузера достаточно выполнить:

```js
localStorage.setItem('GET-api-v1-module1-list', 'http400');
```

После этого следующий запрос к этому эндпоинту получит от мока ответ с кодом 400 (если в моке для этого сценария так реализовано).

### Прокси и запуск

- **Dev-сервер** (webpack-dev-server) настроен так, что запросы с путём **`/api/`** проксируются на мок-сервер: `target: 'http://localhost:9001'` (см. `webpack/webpack.devserver.ts`).
- Мок-сервер нужно запускать отдельно (в своём репозитории — `npm start`). Базовый путь к API на фронте задаётся через `API_PATH` в `.env.development` (например, `api/v1/`); при прокси запрос уходит на тот же путь на порт 9001.

Итог: приложение открывается по HTTPS с dev-сервера, запросы к `/api/...` уходят на мок; в `develop.html` к ним автоматически добавляется `x-scenario` из `localStorage` по ключу `{METHOD}-{path}` (path из `resource` со слэшами, заменёнными на дефисы).
...
- **Обязательные тесты для нового кода:** любая новая **прикладная логика** (редюсеры, thunks, селекторы, API-слой, утилиты, `appInitializer`, формы с валидацией, страницы и контейнеры с поведением) сопровождается тестами в `__tests__` или рядом (`*.test.ts` / `*.test.tsx`). Исключения допустимы только для чистых типовых файлов (только типы/интерфейсы без исполняемого кода) и тонких barrel-`index.ts`, если они не содержат логики. Для нового UI-компонента с разметкой — минимум рендер или сценарий взаимодействия; для компонента только из Storybook без логики допускается опора на stories, но предпочтительно и короткий unit-тест.
- **Покрытие:** команда `npm run test:coverage` проверяет пороги из `jest.config.js` (`coverageThreshold`). Целевые минимумы: **не менее 80%** по **строкам (`lines`)** и **операторам (`statements`)** по глобальному отчёту; для **функций** и **веток** заданы отдельные пороги (ниже из‑за lazy-импортов, styled-тем и заглушек). Набор файлов для отчёта задаётся `collectCoverageFrom`: из покрытия исключаются все **`index.ts`** (barrel/re-export), а также stories, `src/types/**` и корневой `src/index.tsx`. Точки входа страниц остаются в отчёте как **`index.tsx`** (там объявляются `routes`). При добавлении логики в новые файлы покрывайте их тестами. В тестах динамического обнаружения страниц/модулей можно задать карты `globalThis.__jestWebpackPages` и `globalThis.__jestWebpackModules` (см. `src/App/__tests__/appInitializer.test.ts`). В среде без webpack используется fallback в `appInitializer.ts` (`webpackRequireContext`); в production-сборке по-прежнему вызывается `require.context`.
...
- [ ] Тесты для новой логики (store, API, формы, страницы, утилиты) и при необходимости UI; `npm run test` и `npm run test:coverage` проходят пороги из `jest.config.js`.
...
**Error Boundary** перехватывает ошибки рендера в дочерних компонентах и показывает запасной UI вместо падения всего дерева. Компоненты живут в `src/shared/components/ErrorBoundary/` и подключаются через алиас `@components`.

### Что перехватывает и что нет

| Перехватывает | Не перехватывает |
|---------------|------------------|
| Ошибки в рендере дочерних компонентов | Ошибки в **event handlers** (onClick, onChange и т.д.) |
| Ошибки в lifecycle-методах дочерних | **Асинхронный код** (setTimeout, fetch, промисы) |
| Ошибки в конструкторах дочерних | Ошибки **внутри самого Error Boundary** |


Ошибки в обработчиках событий и в асинхронном коде нужно обрабатывать обычным `try/catch` или глобальными обработчиками (`window.onerror`, `unhandledrejection`).

### Компоненты

- **`ErrorBoundary`** — классовый компонент-обёртка. При ошибке в потомках переключает состояние и рендерит fallback.
- **`ErrorFallback`** — UI по умолчанию: заголовок «Ошибка в …», сообщение ошибки и кнопка «Повторить». Используется, если свой `fallback` не передан.
...
| `fallback` | `ReactNode` | Опционально. Кастомный UI при ошибке. Если не передан — показывается `ErrorFallback`. |
| `level` | `'app' \| 'page' \| 'module'` | Уровень границы: для подписи в fallback и в логах. По умолчанию `'module'`. |
| `componentName` | `string` | Опционально. Идентификатор границы (например, имя модуля) для логов. |

Логирование выполняется **один раз внутри** `componentDidCatch`: в консоль выводится префикс `[ErrorBoundary level componentName]`, объект ошибки и `errorInfo.componentStack`. Передавать callback снаружи не нужно. В production-сборке `componentStack` обычно пустой; для продакшена опирайтесь на `error` и `error.stack`.

### Уровни и размещение

Рекомендуется многоуровневая обёртка:

1. **Уровень приложения (`level="app"`)** — в корне приложения (например, в `index.tsx`). Ловит любую необработанную ошибку рендера и не даёт показать пустой экран.
2. **Уровень страницы (`level="page"`)** — вокруг контента страницы или layout с `<Outlet />`. При ошибке падает только эта страница, остальная оболочка (меню, хедер) может оставаться.
3. **Уровень модуля (`level="module"`)** — вокруг отдельного модуля или блока. Падает только этот блок, остальная страница работает.

Чем ниже уровень, тем меньше «зона поражения» и тем проще пользователю продолжить работу (например, перейти на другую вкладку или обновить только проблемный блок).
...
**Отдельный модуль (например, с lazy):**
...
<ErrorBoundary
  level="module"
  fallback={<div>Не удалось загрузить блок. <a href="/">На главную</a></div>}>
  <HeavyWidget />
</ErrorBoundary>
```

**Повторная попытка:** если используется стандартный `ErrorFallback`, кнопка «Повторить» сбрасывает состояние границы (`hasError: false`), и дочерние компоненты рендерятся заново. Для кастомного `fallback` свою кнопку «Повторить» можно реализовать через ключ (например, `key={retryCount}`) и перемонтирование дерева.
...
**Опциональность.** Этот раздел и [§18](#18-реализация-фичи-и-поставка-ветка-pr-закрытие-задач) **не входят** в обязательные инварианты проекта. Это **рекомендуемый рабочий процесс** для сценариев с трекером (Gitea и т.п.). Им следует пользоваться **только если пользователь прямо указал это в промпте** — например: «составь план в репозитории по §17», «создай milestone и issues как в документации». Без такого указания не требуется заводить milestone, issues или wiki от имени процесса «по умолчанию».
...
**Опциональность.** Как и [§17](#17-планирование-фичи-в-трекере-milestone-и-issues), этот раздел **опционален**: цикл «отдельная ветка → push → PR → закрытие issues в трекере» применяется **только при явной просьбе пользователя в промпте** (например: «сделай в отдельной ветке и открой PR», «закрой issues после реализации», «следуй §18»). Обязательные для кода требования по-прежнему задаются **Architecture Invariants**, **Quality Gates** и разделами про модули/тесты; §18 не заменяет их и не требует git/PR без запроса.
